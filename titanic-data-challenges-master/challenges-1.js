...
const getTotalPassengers = (data) => {
  const total = data.reduce((acc) => acc + 1, 0);
  return total;
};
...
const getSurvivorCount = (data) => {
  const survived = data
    .filter((passenger) => passenger.fields.survived === 'Yes')
    .reduce((acc) => acc + 1, 0);
  return survived;
};
...
const getCasualityCount = (data) => {
  const notSurvived = data
    .filter((passenger) => passenger.fields.survived === 'No')
    .reduce((acc) => acc + 1, 0);
  return notSurvived;
};
...
const countPassengersInClass = (data, pclass) => {
  const count = data
    .filter((passenger) => passenger.fields.pclass === pclass)
    .reduce((acc) => acc + 1, 0);
  return count;
};
...
const getMinAge = (data) => {
  const minAge = data
    ...
    .reduce((minage, passenger) => {
      if (minage > passenger.fields.age) {
        minage = passenger.fields.age;
      }
      return minage;
    });
  return minAge;
...
const getMaxAge = (data) => {
  const maxAge = data
    .filter((passenger) => passenger.fields.age != undefined)
    .reduce((maxage, passenger) => {
      if (maxage < passenger.fields.age) {
        maxage = passenger.fields.age;
      }
      return maxage;
    });
  return maxAge;
...
const getEmbarkedCount = (data, embarked) => {
  const embarkedTotal = data
    .filter((passenger) => passenger.fields.embarked === embarked)
    .reduce((acc) => acc + 1, 0);
  return embarkedTotal;
};
...
const getMinFare = (data) => {
  const minFare = data
    ...
    .reduce((minfare, passenger) => {
      if (minfare > passenger.fields.fare) {
...
const getMaxFare = (data) => {
  const maxFare = data
    .filter((passenger) => passenger.fields.fare != undefined)
    .reduce((maxfare, passenger) => {
      if (maxfare < passenger.fields.fare) {
...
const getPassengersByGender = (data, gender) => {
  const gender = data
    .filter((passenger) => passenger.fields.gender === gender)
    .reduce((acc) => acc + 1, 0);
  return gender;
};
...
const getSurvivorsByGender = (data, gender) => {
  const genderSurvived = data
    .filter(
      (passenger) =>
        passenger.fields.gender === gender &&
        passenger.fields.survived === 'Yes'
    )
    .reduce((acc) => acc + 1, 0);
  return genderSurvived;
};
...
const getCasualitiesByGender = (data, gender) => {
  const genderNotSurvived = data
    .filter(
      (passenger) =>
        passenger.fields.gender === gender && passenger.fields.survived === 'No'
    )
    .reduce((acc) => acc + 1, 0);
  return genderNotSurvived;
};
...
const getTotalFare = (data) => {
  const totalFare = data
    .filter((passenger) => passenger.fields.fare != undefined)
    .reduce((acc, passenger) => acc + passenger.fields.fare, 0);
  return totalFare;
};
...
const getMedianFare = (data) => {
  ...
  const median = Math.floor(fares.length / 2),
    nums = [...fares].sort((a, b) => a - b);
  return arr.length % 2 !== 0
    ? nums[median]
    : (nums[median - 1] + nums[median]) / 2;
};
...
const getAverageAge = (data) => {
  const ages = data
    .filter((passenger) => passenger.fields.age != undefined)
    .reduce((acc, passenger) => acc + passenger.fields.age, 0);
  const passengers = data.reduce((acc) => acc + 1, 0);
  const avgAge = ages / passengers;
  return avgAge;
};
...
const getAverageAgeByGender = (data, gender) => {
  const totalByGender = data
    .filter(
      (passenger) =>
        passenger.fields.age != undefined && passenger.fields.gender === gender
    )
    .reduce((acc, passenger) => acc + passenger.fields.age, 0);
  const total = data.reduce((acc) => acc + 1, 0);
  const avg = totalByGender / total;
  return avg;
};
