...
console.log('5 -------');
const ratingList = users.map(function (users) {
  return `${users.rating}`;
});
...
console.log('6 -------');
const usersFullList = users.map(function (users) {
...
console.log('9 -------');
const rating2850 = users.filter(function (users) {
...
const userArray = users.reduce(
  (acc, users) => `${acc} ${users.name} : ${users.rating},`,
  ''
);
